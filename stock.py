...
def format_large_numbers(num):
    if num >= 1e12:
        return f"{num / 1e12:.3g}T"
    elif num >= 1e9:
        return f"{num / 1e9:.3g}B"
    elif num >= 1e6:
...
def get_stock_data(ticker):
    ...
    data = {
        ...
        "Market Cap": format_large_numbers(stock.info.get('marketCap', 'N/A')) if isinstance(stock.info.get('marketCap'), (int, float)) else 'N/A',
        "Volume": format_large_numbers(stock.info.get('volume', 'N/A')) if isinstance(stock.info.get('volume'), (int, float)) else 'N/A',
        ...
        "Revenue": format_large_numbers(stock.info.get('totalRevenue', 'N/A')) if isinstance(stock.info.get('totalRevenue'), (int, float)) else 'N/A',
        "Net Income": format_large_numbers(stock.info.get('netIncomeToCommon', 'N/A')) if isinstance(stock.info.get('netIncomeToCommon'), (int, float)) else 'N/A',
